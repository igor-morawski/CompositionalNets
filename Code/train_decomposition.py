from model_decomposition import DecomOccNet
...
out_dir = model_save_dir + 'decomposition_train_{}_{}_a{}_b{}_vc{}_mix{}_occlikely{}_vc{}_lr_{}_{}_pretrained{}_epochs_{}_occ{}_backbone{}_{}/'.format(date,
...
def train(model, train_data, val_data, epochs, batch_size, learning_rate, savedir, alpha=3,beta=3, vc_flag=True, mix_flag=False):
	...
	for name, param in model.decompose.named_parameters():
		param.requires_grad = False

	for name, param in model.adapt_I.named_parameters():
...
if __name__ == '__main__':
	...
	net = DecomOccNet(extractor, weights, vMF_kappa, occ_likely, mix_models, bool_mixture_bg=bool_mixture_model_bg,compnet_type=compnet_type,num_mixtures=num_mixtures, vc_thresholds=cfg.MODEL.VC_THRESHOLD)
	if bool_load_pretrained_model:
		net.load_state_dict(torch.load(pretrained_file, map_location='cuda:{}'.format(device_ids[0]))['state_dict'])
	

	net.decompose.load_state_dict(torch.load(os.path.join("models", "Decom9200.tar")))
