...
class DecomOccNet(nn.Module):
    def __init__(self, backbone, weights, vMF_kappa, occ_likely, mix_model,
        ...
        self.adapt_I = nn.Sequential(nn.Conv2d(1,1, (1,1)), nn.Sigmoid())

    def forward(self, x):
        vgg_feat = self.backbone(x)
        R, I = self.decompose(x)
        vc_activations = self.conv1o1(vgg_feat)
        vmf_activations = self.activation_layer(vc_activations)
        B, C, H, W = vmf_activations.size()
        I = self.adapt_I(nn.functional.adaptive_max_pool2d(I, (H, W)))
        mix_likeli = self.pointwiseinference(vmf_activations, I)
